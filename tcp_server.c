...
struct sockaddr_in svaddr = {0};//服务器Socket地址
...
{
	struct sockaddr_in *claddr = (struct sockaddr_in *)arg;
...
{
	int err;
	err = pthread_create(&ntid1,NULL,tcp_fun,(void *)claddr1);
	if(err!=0)
	{
		printf("can't create thread1:%s\n",strerror(err));	
	}
	printf("create thread1 sucess!\n");
	err = pthread_create(&ntid2,NULL,tcp_fun,(void *)claddr2);
	if(err!=0)
	{
		printf("can't create thread1:%s\n",strerror(err));	
	}
